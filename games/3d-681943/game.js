window.__iis_game_boot_ok = true;
      const CONFIG = {"mode": "arcade_generic", "title": "풀3D 비행기 조종 시뮬레이터 Infinite", "genre": "arcade", "slug": "3d-681943", "accentColor": "#0EA5E9", "viewportWidth": 1280, "viewportHeight": 720, "safeAreaPadding": 24, "minFontSizePx": 14, "textOverflowPolicy": "ellipsis-clamp", "assetPack": {"name": "neon_arcade", "bg_top": "#111827", "bg_bottom": "#050915", "horizon": "#0f172a", "track": "#111827", "hud_primary": "#e2e8f0", "hud_muted": "#93c5fd", "player_primary": "#22C55E", "player_secondary": "#0f172a", "enemy_primary": "#F8FAFC", "enemy_elite": "#f97316", "boost_color": "#22d3ee", "accent": "#0EA5E9", "particle": "#22c55e", "sfx_profile": "synth", "sprite_profile": "neon"}, "player_hp": 2, "player_speed": 380, "player_attack_cooldown": 0.25, "enemy_hp": 1, "enemy_speed_min": 150, "enemy_speed_max": 280, "enemy_spawn_rate": 0.4, "time_limit_sec": 90, "base_score_value": 50};
      const ASSET = {
        name: "neon_arcade",
        bg_top: "#08122f",
        bg_bottom: "#050915",
      ...
      const gl = webglCanvas.getContext("webgl", { antialias: true });
      ...
      const state = {
        ...
        lastTime: 0,
        player: { x: canvas.width * 0.5, y: canvas.height * 0.8, w: 36, h: 56, vx: 0, vy: 0, visualVx: 0, visualVy: 0, lane: 1 },
        enemies: [],
        bullets: [],
        particles: [],
        spawnTimer: 0,
      ...
      function resetState() {
        ...
        state.lastTime = 0;
        state.player = { x: canvas.width * 0.5, y: canvas.height * 0.8, w: 36, h: 56, vx: 0, vy: 0, visualVx: 0, visualVy: 0, lane: 1 };
        state.enemies = [];
        state.bullets = [];
        state.particles = [];
        state.spawnTimer = 0;
      ...
      function ensureWebglRuntime() {
        ...
        const frag = `
          ...
          void main() {
            ...
            vec3 ro = vec3(0.0, 1.0, uTime * uSpeed * 0.03);
            vec3 rd = normalize(vec3(uv, 1.0));
            
            float pitch = 0.1;
            float cs = cos(pitch), sn = sin(pitch);
            rd.yz = mat2(cs, -sn, sn, cs) * rd.yz;
            ...
            if (rd.y < 0.0) {
                ...
                vec2 grid = abs(fract(p.xz * 0.2) - 0.5);
                float line = smoothstep(0.48, 0.5, max(grid.x, grid.y));
                
                float flow = smoothstep(0.45, 0.5, abs(fract(p.z * 0.05 - uTime * 2.0) - 0.5));
                
                float depth = exp(-t * 0.015);
                vec3 gridCol = mix(uAccent * 0.5, uAccent * 2.0, flow);
                col = mix(vec3(0.01, 0.02, 0.05), gridCol, line) * depth;
            } else {
                float glow = exp(-rd.y * 6.0);
                col += uAccent * glow * 0.5;
                
                float star = fract(sin(dot(floor(rd.xy * 300.0), vec2(12.9898, 78.233))) * 43758.5453);
                if (star > 0.99) col += vec3(1.0) * exp(-rd.y * 3.0) * (0.5 + 0.5 * sin(uTime * 5.0 + star * 100.0));
            }
      ...
      function update(dt) {
        ...
        if (MODE_IS_3D_RUNNER) {
          ...
          state.player.visualVx = (state.player.visualVx || 0) + (state.player.vx - (state.player.visualVx || 0)) * dt * 15;
          state.player.visualVy = (state.player.visualVy || 0) + (state.player.vy - (state.player.visualVy || 0)) * dt * 15;
          ...
          if (state.running) {
            state.particles.push({
                x: state.player.x + state.player.w / 2 - 10,
                y: state.player.y + state.player.h - 10,
                life: rand(0.15, 0.3), t: 0, color: ASSET.boost_color,
                vx: rand(-15, 15), vy: rand(150, 250),
                size: rand(2, 4)
            });
            state.particles.push({
                x: state.player.x + state.player.w / 2 + 10,
          ...
          for (const e of state.enemies) {
            ...
            if (rectsOverlap(state.player, e)) {
              ...
              playSfx("damage");
              e.y = canvas.height + 999;
              burst(state.player.x + state.player.w / 2, state.player.y + state.player.h / 2, ASSET.enemy_primary, 15);
            }
          }
      ...
      function draw(dt = 0.016) {
        ...
        if (state.run.shake > 0) {
          const shakePx = 8 * state.run.shake;
        ...
        if (MODE_IS_3D_RUNNER) {
          ...
          const webglRendered = CONFIG.mode === "webgl_three_runner" ? renderWebglBackground(dt) : false;
          if (!webglRendered) {
          ...
          const webglRendered = CONFIG.mode === "arcade_generic" ? renderWebglBackground(dt) : false;
          if (!webglRendered) {
        ...
        if (MODE_IS_3D_RUNNER) {
          ...
          if (CONFIG.mode === "topdown_roguelike_shooter") {
            ...
            const px = state.player.x + state.player.w / 2;
            const py = state.player.y + state.player.h / 2;
            const tilt = (state.player.visualVx || 0) * 0.4;
            const pitch = (state.player.visualVy || 0) * 0.15;
            ...
            ctx.fillStyle = ASSET.player_primary;
            ctx.beginPath();
            ctx.moveTo(0, -state.player.h/2);
            ctx.lineTo(state.player.w/6, -state.player.h/4);
            ctx.lineTo(state.player.w/2, state.player.h/6);
            ctx.lineTo(state.player.w/2, state.player.h/4);
            ctx.lineTo(state.player.w/6, state.player.h/6);
            ctx.lineTo(state.player.w/8, state.player.h/2 - 5);
            ctx.lineTo(state.player.w/3, state.player.h/2);
            ctx.lineTo(state.player.w/3, state.player.h/2 + 5);
            ctx.lineTo(0, state.player.h/2);
            ctx.lineTo(-state.player.w/3, state.player.h/2 + 5);
            ctx.lineTo(-state.player.w/3, state.player.h/2);
            ctx.lineTo(-state.player.w/8, state.player.h/2 - 5);
            ctx.lineTo(-state.player.w/6, state.player.h/6);
            ...
            ctx.shadowBlur = 0;
            ctx.beginPath();
            ctx.moveTo(0, -state.player.h/4);
            ctx.lineTo(state.player.w/8, 0);
            ctx.lineTo(-state.player.w/8, 0);
            ctx.closePath();
            ctx.fill();
      ...
      function endGame() {
        ...
        overlayText.textContent = `최종 점수 ${Math.floor(state.score)} · 콤보 x${Math.max(1, state.run.combo.toFixed(1))} · 빌드(${buildSummary})`;
        playSfx("gameover");
